...
func TestNewSettings(t *testing.T) {
	...
	assert.Equal(t, settings.DBConnectionString, "/app/data/vega.db?_journal_mode=WAL&_busy_timeout=5000&_foreign_keys=ON&_cache_size=10000&_synchronous=NORMAL", "Expected default DB connection string to be enhanced")
...
func TestGetEnv(t *testing.T) {
	...
	t.Run("should return overridden DB_CONNECTION_STRING from environment variable", func(t *testing.T) {
		value := getEnv("DB_CONNECTION_STRING", "/app/data/vega.db?_journal_mode=WAL&_busy_timeout=5000&_foreign_keys=ON&_cache_size=10000&_synchronous=NORMAL")
