...
func TestRole(t *testing.T) {
	t.Run("should return correct string for ADMIN role", func(t *testing.T) {
		role := ADMIN
		assert.Equal(t, "Admin", role.String(), "Expected role string to be 'Admin'")
	})

	t.Run("should return correct string for STANDARD role", func(t *testing.T) {
		role := STANDARD
		assert.Equal(t, "Standard", role.String(), "Expected role string to be 'Standard'")
	})

	t.Run("should return 'Unknown' for unknown role", func(t *testing.T) {
		role := Role(999)
		assert.Equal(t, "Unknown", role.String(), "Expected role string to be 'Unknown'")
	})
}
