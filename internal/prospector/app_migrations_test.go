...
func TestAppMigrationIntegration(t *testing.T) {
	...
	dbPath := filepath.Join(dbDir, "test_prospector.db")
...
func TestAppSkipsMigrationsInTestMode(t *testing.T) {
	tempDir, err := os.MkdirTemp("", "app-migrations-skip-test")
	require.NoError(t, err)
	defer os.RemoveAll(tempDir)
